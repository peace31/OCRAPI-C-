	...
	stoppedByUser:(BOOL)stoppedByUser
{
    ...
    for( DetectionDictInfo* dict in textCaptureController.mDictInfoList){
        if( dict.ocrDict.resValue != NULL ){
            NSDictionary *objItem = [NSDictionary dictionaryWithObjectsAndKeys:dict.ocrDict.name, @"name", dict.ocrDict.resValue, @"value", nil];
            [arrData addObject:objItem];
        }
    }
    NSDictionary* result = @{ @"Detected Items" : [arrData copy] };
