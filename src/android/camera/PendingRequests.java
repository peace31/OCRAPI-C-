...
public class PendingRequests {
    private static final String TAG = PendingRequests.class.getSimpleName();
     ...
     *                      request is not found
     */
    public synchronized Request getLastRecognizeRequest() {
        // Check to see if this request was saved
        int lastReqCode = -1;
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            if( r.action == Capture1.ACTION_RECOGNIZE_ID
                    && lastReqCode < requestCode )
                lastReq = r;
        }
        return lastReq;
    }
    ...
    public synchronized void resolveWithSuccess(Request req) {
        this.resolveWithSuccess(req, true);
    }
    /**
     * Removes the request from the array of pending requests and sends a successful plugin result
     * to the CallbackContext that contains the result of the request
     * @param req   The request to be resolved
     */
    public synchronized void resolveWithSuccess(Request req, boolean shouldBeFinish) {
    ...
    public synchronized Bundle toBundle() {
        ...
        if (requests.size() > 1) {
            ...
            LOG.w(TAG, "More than one media capture request pending on Activity destruction. Some requests will be dropped!");
        }
    ...
    public class Request {
        ...
        private Request(Bundle bundle, CallbackContext callbackContext, int requestCode) {
            ...
            try {
                ...
                this.options = new JSONObject(bundle.getString(KEY_OPTIONS));
            } catch(JSONException e) {
                // This should never be caught
                LOG.e(TAG, "Error parsing results for request from saved bundle", e);
            }
        }
        ...
        private Bundle toBundle() {
            ...
            bundle.putString(KEY_OPTIONS, this.options.toString());
            return bundle;
        }
    }
}
